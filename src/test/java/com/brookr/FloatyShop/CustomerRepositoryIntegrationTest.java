...
public class CustomerRepositoryIntegrationTest {
    ...
    private static final int EXPECTED_ORDERS = 50;
    ...
    public void readWriteTestCase() {
        Customer supa = new Customer(EXPECTED_NAME, EXPECTED_ORDERS);
        repository.save(supa);

        List<Customer> result = (List<Customer>) repository.findAll();

        assertTrue("Not empty", result.size() > 0);
        assertEquals("Contains customer with expected number of orders", result.get(0).getOrders(), EXPECTED_ORDERS);
    }
}
